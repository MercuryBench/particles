...
class Bootstrap_RaoBlackwell_old(particles.FeynmanKac):
  ...
  def M0(self, N):
    ws = self.PW0.rvs(size=N)
    MC0s = smp.FancyList([kalman.MeanAndCov(mean=self.m0(ws[j]), cov=self.cov0(ws[j])) for j in range(N)])
    ...
    for j in range(J):
      MCs_filter[j], logpyts[j] = kalman.filter_step(self.H(ws[j]), self.Sigma(ws[j]), MC0s[j], self.data[0])
    return smp.ThetaParticles(w=ws, MC=smp.FancyList(MCs_filter), logGs=logpyts)
  
  def M(self, t, xp):
    J = xp.N
    ws_old = xp.w
    ws_new = self.PW(t, ws_old).rvs(J)    
    ...
    for j, part in enumerate(xp):
      MC_old = part['MC']
      MC_pred[j] = kalman.predict_step(self.A(part['w']), self.Gamma(part['w']), MC_old, self.a(part['w']))
      MC_filter[j], logpyts[j] = kalman.filter_step(self.H(part['w']), self.Sigma(part['w']), MC_pred[j], self.data[t], self.h(part['w']))
...
if __name__ == "__main__":
    ...
    if case == 1: 
        ...
        my_pmmh = mcmc.PMMH(ssm_cls=DiffDiff, fk_cls=Bootstrap_RaoBlackwell, prior=my_prior, data=ys, Nx=250,  niter=100, verbose=1000)
        my_pmmh.run()
